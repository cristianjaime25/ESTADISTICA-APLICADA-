...
```{r}
#| code-fold: true
# Ejercicio 1: Máquina produce piezas defectuosas
p <- 0.02  # probabilidad de defectuosa
q <- 1 - p # probabilidad de no defectuosa

# a) Probabilidad de pieza no defectuosa
cat("a) Probabilidad de pieza no defectuosa: q =", q, "\n\n")

# b) Media y varianza de la distribución Bernoulli
media_bern <- p
varianza_bern <- p * q

cat("b) Parámetros de la distribución Bernoulli:\n")
cat("   Media: μ =", media_bern, "\n")
cat("   Varianza: σ² =", varianza_bern, "\n\n")

# c) Probabilidad de exactamente 2 defectuosas en 10 piezas (Binomial)
n <- 10
k <- 2
prob_2def <- dbinom(k, size = n, prob = p)

cat("c) P(exactamente 2 defectuosas en 10 piezas):", prob_2def, "\n")
cat("   Porcentaje:", round(prob_2def * 100, 3), "%\n")
```
...
```{r}
#| code-fold: true
# Ejercicio 2: Examen de opción múltiple con 4 opciones
p_examen <- 1/4  # 1 correcta de 4 opciones
q_examen <- 3/4

# a) Modelo Bernoulli
cat("a) Modelo de Bernoulli para una pregunta:\n")
cat("   X = 1 (acierta), X = 0 (falla)\n")
cat("   X ~ Bernoulli(p = 1/4)\n\n")

# b) Probabilidad de acertar
cat("b) Probabilidad de acertar una pregunta:\n")
cat("   p =", p_examen, "=", round(p_examen * 100, 2), "%\n\n")

# c) Parámetros estadísticos
media_examen <- p_examen
varianza_examen <- p_examen * q_examen
desv_examen <- sqrt(varianza_examen)

cat("c) Parámetros estadísticos:\n")
cat("   Media: μ =", media_examen, "\n")
cat("   Varianza: σ² =", varianza_examen, "\n")
cat("   Desviación estándar: σ =", desv_examen, "\n")
```
...
# Ejercicio 3: Se lanza una moneda 15 veces
n_moneda <- 15
p_moneda <- 0.5

# a) P(exactamente 8 caras)
prob_8caras <- dbinom(8, size = n_moneda, prob = p_moneda)

cat("a) Probabilidad de obtener exactamente 8 caras:\n")
cat("   P(X = 8) =", prob_8caras, "\n")
cat("   Porcentaje:", round(prob_8caras * 100, 2), "%\n\n")

# b) P(al menos 12 caras)
prob_almenos12 <- 1 - pbinom(11, size = n_moneda, prob = p_moneda)
# O también: sum(dbinom(12:15, size = n_moneda, prob = p_moneda))

cat("b) Probabilidad de obtener al menos 12 caras:\n")
cat("   P(X ≥ 12) =", prob_almenos12, "\n")
cat("   Porcentaje:", round(prob_almenos12 * 100, 3), "%\n\n")

# c) Media y desviación estándar
media_moneda <- n_moneda * p_moneda
desv_moneda <- sqrt(n_moneda * p_moneda * (1 - p_moneda))

cat("c) Parámetros de la distribución:\n")
cat("   Media: μ =", media_moneda, "\n")
cat("   Desviación estándar: σ =", desv_moneda, "\n\n")

# Gráfica de la distribución
x_vals <- 0:n_moneda
probs <- dbinom(x_vals, size = n_moneda, prob = p_moneda)
...
ggplot(df_moneda, aes(x = caras, y = probabilidad)) +
  geom_col(fill = "#2E86AB", alpha = 0.7) +
  geom_vline(xintercept = media_moneda, color = "#E63946", 
             linetype = "dashed", linewidth = 1.2) +
  labs(title = "Distribución Binomial: 15 lanzamientos de moneda",
       subtitle = paste("p =", p_moneda, "| Media (línea roja) =", media_moneda),
       x = "Número de caras",
       y = "Probabilidad") +
  theme_minimal()
```
...
# Ejercicio 4: Medicamento con 85% de efectividad, 20 pacientes
n_pacientes <- 20
p_efectividad <- 0.85

# a) P(exactamente 18 se curen)
prob_18curados <- dbinom(18, size = n_pacientes, prob = p_efectividad)

cat("a) Probabilidad de que se curen exactamente 18 pacientes:\n")
cat("   P(X = 18) =", prob_18curados, "\n")
cat("   Porcentaje:", round(prob_18curados * 100, 2), "%\n\n")

# b) P(entre 15 y 19 se curen)
prob_15a19 <- sum(dbinom(15:19, size = n_pacientes, prob = p_efectividad))
# O también: pbinom(19, n_pacientes, p_efectividad) - pbinom(14, n_pacientes, p_efectividad)

cat("b) Probabilidad de que se curen entre 15 y 19 pacientes:\n")
cat("   P(15 ≤ X ≤ 19) =", prob_15a19, "\n")
cat("   Porcentaje:", round(prob_15a19 * 100, 2), "%\n\n")

# c) Número esperado de pacientes curados
esperado <- n_pacientes * p_efectividad

cat("c) Número esperado de pacientes que se curan:\n")
cat("   E[X] = n × p =", esperado, "pacientes\n\n")

# Gráfica de la distribución
x_pac <- 0:n_pacientes
probs_pac <- dbinom(x_pac, size = n_pacientes, prob = p_efectividad)

df_pacientes <- data.frame(
  curados = x_pac,
  probabilidad = probs_pac
)

ggplot(df_pacientes, aes(x = curados, y = probabilidad)) +
  geom_col(fill = "#A23B72", alpha = 0.7) +
  geom_vline(xintercept = esperado, color = "#FF6B35", 
             linetype = "dashed", linewidth = 1.2) +
  annotate("rect", xmin = 15, xmax = 19, ymin = 0, ymax = Inf,
           alpha = 0.2, fill = "green") +
  labs(title = "Distribución de pacientes curados con medicamento",
       subtitle = paste("Efectividad =", p_efectividad * 100, "% | Esperado (línea naranja) =", esperado),
       x = "Número de pacientes curados",
       y = "Probabilidad") +
  theme_minimal()
```
...
cat("TEOREMA: SUMA DE DISTRIBUCIONES BINOMIALES\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n\n")

cat("ENUNCIADO:\n")
cat("Si X ~ Bin(n, p) y Y ~ Bin(m, p) son variables aleatorias independientes,\n")
cat("entonces X + Y ~ Bin(n+m, p)\n\n")

cat("DEMOSTRACIÓN (usando funciones generadoras):\n")
cat("--------------------------------------------------\n")
cat("La función generadora de probabilidad de X ~ Bin(n,p) es:\n")
cat("  G_X(t) = (1-p+pt)^n\n\n")

cat("Para Y ~ Bin(m,p):\n")
cat("  G_Y(t) = (1-p+pt)^m\n\n")

cat("Como X e Y son independientes:\n")
cat("  G_(X+Y)(t) = G_X(t) × G_Y(t)\n")
cat("             = (1-p+pt)^n × (1-p+pt)^m\n")
cat("             = (1-p+pt)^(n+m)\n\n")

cat("Esta es la función generadora de Bin(n+m, p), por lo tanto:\n")
cat("  X + Y ~ Bin(n+m, p) ✓\n\n")

cat("=" %>% rep(70) %>% paste(collapse = ""), "\n")
cat("VERIFICACIÓN POR SIMULACIÓN\n")
cat("=" %>% rep(70) %>% paste(collapse = ""), "\n\n")
...
# Simulación de X ~ Bin(n1, p)
X_sim <- rbinom(n_sim, size = n1, prob = p)

# Simulación de Y ~ Bin(n2, p)
Y_sim <- rbinom(n_sim, size = n2, prob = p)

# Suma X + Y
XplusY_sim <- X_sim + Y_sim

# Simulación directa de Z ~ Bin(n1+n2, p)
Z_sim <- rbinom(n_sim, size = n1 + n2, prob = p)

# Cálculos teóricos
media_teorica <- (n1 + n2) * p
varianza_teorica <- (n1 + n2) * p * (1 - p)

# Cálculos de simulación
media_XplusY <- mean(XplusY_sim)
media_Z <- mean(Z_sim)

varianza_XplusY <- var(XplusY_sim)
varianza_Z <- var(Z_sim)

# Resultados
cat("PARÁMETROS:\n")
cat("  X ~ Bin(", n1, ",", p, ")\n", sep = "")
cat("  Y ~ Bin(", n2, ",", p, ")\n", sep = "")
cat("  Número de simulaciones:", format(n_sim, big.mark = ","), "\n\n")

cat("MEDIAS:\n")
cat("  Teórica (n+m)×p =", media_teorica, "\n")
cat("  X+Y (simulación) =", round(media_XplusY, 4), "\n")
cat("  Z ~ Bin(n+m,p) (simulación) =", round(media_Z, 4), "\n")
cat("  Diferencia |X+Y - Z| =", abs(media_XplusY - media_Z), "\n\n")

cat("VARIANZAS:\n")
cat("  Teórica (n+m)×p×(1-p) =", varianza_teorica, "\n")
cat("  X+Y (simulación) =", round(varianza_XplusY, 4), "\n")
cat("  Z ~ Bin(n+m,p) (simulación) =", round(varianza_Z, 4), "\n")
cat("  Diferencia |Var(X+Y) - Var(Z)| =", abs(varianza_XplusY - varianza_Z), "\n\n")

cat("CONCLUSIÓN:\n")
cat("Las medias y varianzas de X+Y y Z ~ Bin(n+m, p) son prácticamente\n")
cat("idénticas, verificando empíricamente el teorema. ✓\n\n")

# Gráfica comparativa
datos_comparacion <- data.frame(
  valores = c(XplusY_sim, Z_sim),
  distribucion = rep(c("X + Y (suma)", "Z ~ Bin(n+m, p)"), 
                     each = n_sim)
)

ggplot(datos_comparacion, aes(x = valores, fill = distribucion)) +
  geom_histogram(alpha = 0.6, bins = 30, position = "identity") +
  geom_vline(xintercept = media_teorica, color = "#E63946", 
             linetype = "dashed", linewidth = 1.2) +
  facet_wrap(~distribucion, ncol = 1) +
  labs(title = "Verificación del Teorema: Suma de Binomiales",
       subtitle = paste("X ~ Bin(", n1, ",", p, "), Y ~ Bin(", n2, ",", p, 
                       ") | Línea roja: media teórica =", media_teorica, sep = ""),
       x = "Número de éxitos",
       y = "Frecuencia") +
  scale_fill_manual(values = c("#2E86AB", "#8B4789")) +
  theme_minimal() +
