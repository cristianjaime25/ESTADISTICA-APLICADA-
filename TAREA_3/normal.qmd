...
mu <- 350
sigma <- 5
especificacion_min <- 340
especificacion_max <- 360
n_muestras <- 1000

# Simular 1000 mediciones de temperatura
set.seed(123)  # Para reproducibilidad
temperaturas <- rnorm(n_muestras, mean = mu, sd = sigma)

# Calcular porcentaje fuera de especificación (fuera de [340, 360])
fuera_especificacion <- sum(temperaturas < especificacion_min | temperaturas > especificacion_max)
porcentaje_fuera <- (fuera_especificacion / n_muestras) * 100
cat("Porcentaje de tiempo fuera de especificación (μ=350, σ=5):", porcentaje_fuera, "%\n")

# Si el proceso se descentra a μ=355
mu_nuevo <- 355
temperaturas_nuevas <- rnorm(n_muestras, mean = mu_nuevo, sd = sigma)
fuera_especificacion_nuevo <- sum(temperaturas_nuevas < especificacion_min | temperaturas_nuevas > especificacion_max)
porcentaje_fuera_nuevo <- (fuera_especificacion_nuevo / n_muestras) * 100
cat("Porcentaje de tiempo fuera de especificación (μ=355, σ=5):", porcentaje_fuera_nuevo, "%\n")

# Calcular σ necesario para 99.9% dentro de especificación
# P(340 < X < 360) = 0.999 con μ=350
# Esto equivale a P(|Z| < 10/σ) = 0.999, donde Z ~ N(0,1)
# El valor de k tal que P(|Z| < k) = 0.999 es k = qnorm(0.9995)
k <- qnorm(0.9995)  # Percentil 99.95% (para 99.9% en dos colas)
sigma_requerido <- 10 / k
cat("σ necesario para 99.9% dentro de especificación:", sigma_requerido, "\n")

# Opcional: Visualización de las distribuciones
par(mfrow = c(1, 2))
hist(temperaturas, freq = FALSE, col = "lightblue", 
     xlab = "Temperatura (°C)", ylab = "Densidad", 
     main = "Distribución original (μ=350, σ=5)")
abline(v = c(especificacion_min, especificacion_max), col = "red", lty = 2)

hist(temperaturas_nuevas, freq = FALSE, col = "lightgreen", 
     xlab = "Temperatura (°C)", ylab = "Densidad", 
     main = "Distribución descentrada (μ=355, σ=5)")
abline(v = c(especificacion_min, especificacion_max), col = "red", lty = 2)
...
5.  Si la meta es superar 45,000 pesos, ¿qué porcentaje de meses se cumple?
    ...
    ```{r}
    # Parámetros de la distribución normal de ventas
    mu <- 50000
    sigma <- 12000

    # Simular ventas de 24 meses
    set.seed(123)  # Para reproducibilidad
    ventas <- rnorm(24, mean = mu, sd = sigma)

    # 1. Simulación de ventas (ya generada arriba)
    cat("Ventas simuladas de 24 meses:\n")
    print(ventas)

    # 2. Probabilidad de ventas superiores a 60,000 pesos
    prob_superior_60000 <- 1 - pnorm(60000, mean = mu, sd = sigma)
    cat("Probabilidad de ventas superiores a 60,000 pesos:", prob_superior_60000, "\n")

    # 3. Número esperado de meses con ventas por debajo de 30,000 pesos
    prob_inferior_30000 <- pnorm(30000, mean = mu, sd = sigma)
    meses_esperados_inferior_30000 <- 24 * prob_inferior_30000
    cat("Número esperado de meses con ventas por debajo de 30,000 pesos:", meses_esperados_inferior_30000, "\n")

    # 4. Rango intercuartílico de las ventas
    q1 <- qnorm(0.25, mean = mu, sd = sigma)
    q3 <- qnorm(0.75, mean = mu, sd = sigma)
    iqr <- q3 - q1
    cat("Rango intercuartílico de las ventas:", iqr, "pesos\n")

    # 5. Porcentaje de meses que superan 45,000 pesos
    prob_superior_45000 <- 1 - pnorm(45000, mean = mu, sd = sigma)
    porcentaje_superior_45000 <- prob_superior_45000 * 100
    cat("Porcentaje de meses que superan 45,000 pesos:", porcentaje_superior_45000, "%\n")

    # Opcional: Visualización de las ventas simuladas
    hist(ventas, freq = FALSE, col = "lightblue", 
         xlab = "Ventas (pesos)", ylab = "Densidad", 
         main = "Distribución de Ventas Mensuales Simuladas")
    curve(dnorm(x, mean = mu, sd = sigma), add = TRUE, col = "red", lwd = 2)
    abline(v = c(30000, 45000, 60000), col = c("green", "orange", "purple"), lty = 2)
    legend("topright", legend = c("Histograma", "PDF teórica", "30k", "45k", "60k"), 
           col = c("lightblue", "red", "green", "orange", "purple"), lty = c(1, 1, 2, 2, 2), 
           cex = 0.7, bty = "n")
    ```
:::
...
5.  ¿Cuál es el percentil 10 de la distribución?
    ...
    ```{r}
    # Parámetros de la distribución normal de duración de baterías
    mu <- 1200
    sigma <- 150

    # Simular la vida útil de 800 baterías
    set.seed(123)  # Para reproducibilidad
    duraciones <- rnorm(800, mean = mu, sd = sigma)

    # 1. Simulación de duraciones (ya generada arriba)
    cat("Duraciones simuladas de 800 baterías (primeras 10):\n")
    print(head(duraciones, 10))

    # 2. Garantía que cubre al 95% de las baterías
    # Esto significa que el 5% fallarán antes de esa garantía, así que es el percentil 5%
    garantia_95 <- qnorm(0.05, mean = mu, sd = sigma)
    cat("Garantía que cubre al 95% de las baterías:", garantia_95, "horas\n")

    # 3. Si se venden 10,000 baterías con garantía de 900 horas, ¿cuántas se espera que fallen?
    prob_fallar_900 <- pnorm(900, mean = mu, sd = sigma)
    esperado_fallar <- 10000 * prob_fallar_900
    cat("Número esperado de baterías que fallen con garantía de 900 horas:", esperado_fallar, "\n")

    # 4. Comparar media y mediana de la simulación con valores teóricos
    media_simulada <- mean(duraciones)
    mediana_simulada <- median(duraciones)
    media_teorica <- mu
    mediana_teorica <- mu  # Para distribución normal, media = mediana
    cat("Media simulada:", media_simulada, "horas\n")
    cat("Mediana simulada:", mediana_simulada, "horas\n")
    cat("Media teórica:", media_teorica, "horas\n")
    cat("Mediana teórica:", mediana_teorica, "horas\n")

    # 5. Percentil 10 de la distribución
    percentil_10 <- qnorm(0.10, mean = mu, sd = sigma)
    cat("Percentil 10 de la distribución:", percentil_10, "horas\n")

    # Opcional: Visualización de las duraciones simuladas
    hist(duraciones, freq = FALSE, col = "lightblue", 
         xlab = "Duración (horas)", ylab = "Densidad", 
         main = "Distribución de Duración de Baterías Simuladas")
    curve(dnorm(x, mean = mu, sd = sigma), add = TRUE, col = "red", lwd = 2)
    abline(v = c(garantia_95, 900, percentil_10), col = c("green", "orange", "purple"), lty = 2)
    legend("topright", legend = c("Histograma", "PDF teórica", "Garantía 95%", "900h", "P10"), 
           col = c("lightblue", "red", "green", "orange", "purple"), lty = c(1, 1, 2, 2, 2), 
           cex = 0.7, bty = "n")
    ```
:::
...
5.  Verifica que la distribución es simétrica alrededor de 0
    ...
    ```{r}
    # Parámetros de la distribución normal de errores
    mu <- 0
    sigma_cuadrado <- 2.5^5  # 2.5 elevado a la quinta potencia
    sigma <- sqrt(sigma_cuadrado)

    # Simular 1000 mediciones de errores
    set.seed(123)  # Para reproducibilidad
    errores <- rnorm(1000, mean = mu, sd = sigma)

    # 1. Simulación de errores (ya generada arriba)
    cat("Errores simulados (primeras 10):\n")
    print(head(errores, 10))

    # 2. Porcentaje de errores entre -5 y +5 unidades
    entre_5 <- sum(errores >= -5 & errores <= 5)
    porcentaje_entre_5 <- (entre_5 / 1000) * 100
    cat("Porcentaje de errores entre -5 y +5 unidades:", porcentaje_entre_5, "%\n")

    # 3. Probabilidad de que un error sea mayor a 6 unidades en valor absoluto
    prob_mayor_6_abs <- 1 - pnorm(6, mean = mu, sd = sigma) + pnorm(-6, mean = mu, sd = sigma)
    cat("Probabilidad de error mayor a 6 unidades en valor absoluto:", prob_mayor_6_abs, "\n")

    # 4. Porcentaje de mediciones precisas (error ≤ 3 unidades en valor absoluto)
    precisas <- sum(abs(errores) <= 3)
    porcentaje_precisas <- (precisas / 1000) * 100
    cat("Porcentaje de mediciones precisas (error ≤ 3 unidades):", porcentaje_precisas, "%\n")

    # 5. Verificar simetría alrededor de 0
    media_simulada <- mean(errores)
    mediana_simulada <- median(errores)
    cat("Media simulada:", media_simulada, "\n")
    cat("Mediana simulada:", mediana_simulada, "\n")
    cat("Diferencia (media - mediana):", media_simulada - mediana_simulada, "\n")
    # Para distribución normal, media ≈ mediana ≈ 0, indicando simetría

    # Opcional: Visualización de los errores simulados
    hist(errores, freq = FALSE, col = "lightblue", 
         xlab = "Error (unidades)", ylab = "Densidad", 
         main = "Distribución de Errores de Medición Simulados")
    curve(dnorm(x, mean = mu, sd = sigma), add = TRUE, col = "red", lwd = 2)
    abline(v = c(-5, 5, -6, 6, -3, 3), col = c("green", "green", "orange", "orange", "purple", "purple"), lty = 2)
    legend("topright", legend = c("Histograma", "PDF teórica", "-5/+5", "-6/+6", "-3/+3"), 
           col = c("lightblue", "red", "green", "orange", "purple"), lty = c(1, 1, 2, 2, 2), 
           cex = 0.7, bty = "n")
    ```
:::
