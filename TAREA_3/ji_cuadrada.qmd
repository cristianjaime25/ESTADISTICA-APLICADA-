...
3.  Para el tamaño de muestra más grande, genera un histograma de la muestra junto con la función de densidad teórica superpuesta.
    ...
    # Definir los tamaños de muestra para la convergencia
    sizes <- c(100, 500, 1000, 2000, 5000, 10000)

    # Para cada grado de libertad
    for (df in dfs) {
      # Calcular media y varianza teóricas
      theoretical_mean <- df
      theoretical_var <- 2 * df
      
      # Vectores para almacenar medias y varianzas muestrales
      sample_means <- numeric(length(sizes))
      sample_vars <- numeric(length(sizes))
      ...
      # Graficar convergencia de la media
      plot(sizes, sample_means, type = "o", col = "blue", 
           xlab = "Tamaño de muestra", ylab = "Media muestral",
           main = paste("Convergencia de la media (df =", df, ")"))
      abline(h = theoretical_mean, col = "red", lty = 2)
      legend("topright", legend = c("Media muestral", "Media teórica"), 
             col = c("blue", "red"), lty = c(1, 2), pch = c(1, NA))
      
      # Graficar convergencia de la varianza
      plot(sizes, sample_vars, type = "o", col = "green", 
           xlab = "Tamaño de muestra", ylab = "Varianza muestral",
           main = paste("Convergencia de la varianza (df =", df, ")"))
      abline(h = theoretical_var, col = "red", lty = 2)
      legend("topright", legend = c("Varianza muestral", "Varianza teórica"), 
             col = c("green", "red"), lty = c(1, 2), pch = c(1, NA))
      
      # Para el tamaño de muestra más grande (10000), generar histograma con densidad teórica
      sample_large <- rchisq(10000, df)
      hist(sample_large, freq = FALSE, breaks = 50, col = "lightblue", 
           xlab = "Valor", main = paste("Histograma y densidad teórica (df =", df, ", n=10000)"))
      curve(dchisq(x, df), add = TRUE, col = "red", lwd = 2)
      legend("topright", legend = c("Histograma", "Densidad teórica"), 
             fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))
    }
    ```
:::
...
```{r}
# Elegir grados de libertad
df <- 15

# Media y varianza teóricas de la distribución chi-cuadrada
mu <- df
sigma <- sqrt(2 * df)

# Tamaños de muestra para verificar el TCL
sizes <- c(10, 30, 50, 100, 500, 1000)

# Número de simulaciones por tamaño de muestra
num_sim <- 10000

# Para cada tamaño de muestra
for (n in sizes) {
  # Generar num_sim muestras de tamaño n
  samples <- matrix(rchisq(n * num_sim, df), nrow = num_sim, ncol = n)
  
  # Calcular la media de cada muestra
  sample_means <- rowMeans(samples)
  
  # Estandarizar las medias: Z = (X_bar - mu) / (sigma / sqrt(n))
  z_scores <- (sample_means - mu) / (sigma / sqrt(n))
  
  # Calcular media y varianza de los Z-scores
  mean_z <- mean(z_scores)
  var_z <- var(z_scores)
  
  # Imprimir resultados numéricos
  cat(sprintf("Tamaño de muestra n = %d:\n", n))
  cat(sprintf("  Media de Z-scores: %.4f (esperado: 0)\n", mean_z))
  cat(sprintf("  Varianza de Z-scores: %.4f (esperado: 1)\n", var_z))
  cat("\n")
  
  # Graficar histograma de Z-scores con densidad normal superpuesta
  hist(z_scores, freq = FALSE, breaks = 50, col = "lightblue", 
       xlab = "Z-score", main = paste("Distribución de Z-scores (n =", n, ")"))
  curve(dnorm(x), add = TRUE, col = "red", lwd = 2)
  legend("topright", legend = c("Histograma", "N(0,1)"), 
         fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))
}
```
:::
...
```{r}
# Elegir grados de libertad (puedes cambiarlo)
k <- 5

# Número de simulaciones
num_sim <- 10000

# Simular sumas de cuadrados de normales estándar
sums <- numeric(num_sim)
for (i in 1:num_sim) {
  # Generar k normales estándar independientes
  normals <- rnorm(k, mean = 0, sd = 1)
  # Elevar al cuadrado y sumar
  sums[i] <- sum(normals^2)
}

# Comparar con distribución chi-cuadrada teórica
# Histograma de las sumas simuladas
hist(sums, freq = FALSE, breaks = 50, col = "lightblue", 
     xlab = "Valor", main = paste("Suma de cuadrados de", k, "normales estándar"))
# Superponer densidad teórica chi-cuadrada
curve(dchisq(x, df = k), add = TRUE, col = "red", lwd = 2)
legend("topright", legend = c("Histograma simulado", "Densidad chi-cuadrada"), 
       fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

# QQ-plot para comparación
qqplot(qchisq(ppoints(num_sim), df = k), sums, 
       xlab = "Cuantiles teóricos chi-cuadrada", 
       ylab = "Cuantiles simulados", 
       main = paste("QQ-plot: Simulado vs Teórico (df =", k, ")"))
abline(0, 1, col = "red", lwd = 2)

# Estadísticas numéricas: media y varianza simuladas vs teóricas
mean_sim <- mean(sums)
var_sim <- var(sums)
mean_theor <- k
var_theor <- 2 * k
...
cat(sprintf("Media simulada: %.4f (teórica: %.4f)\n", mean_sim, mean_theor))
cat(sprintf("Varianza simulada: %.4f (teórica: %.4f)\n", var_sim, var_theor))
```
:::
...
```{r}
# Definir los grados de libertad para verificar la aproximación normal
ns <- c(10, 30, 50, 100)

# Número de simulaciones por grado de libertad
num_sim <- 10000

# Para cada grado de libertad
for (n in ns) {
  # Generar muestra de chi-cuadrada
  samples <- rchisq(num_sim, df = n)
  
  # Media y varianza teóricas de la chi-cuadrada
  mu <- n
  sigma <- sqrt(2 * n)
  
  # Estandarizar: Z = (X - mu) / sigma
  z_scores <- (samples - mu) / sigma
  
  # Graficar histograma de Z-scores con densidad normal superpuesta
  hist(z_scores, freq = FALSE, breaks = 50, col = "lightblue", 
       xlab = "Z-score", main = paste("Aproximación normal para chi-cuadrada (df =", n, ")"))
  curve(dnorm(x), add = TRUE, col = "red", lwd = 2)
  legend("topright", legend = c("Histograma", "N(0,1)"), 
         fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))
  
  # QQ-plot para comparación
  qqplot(qnorm(ppoints(num_sim)), z_scores, 
         xlab = "Cuantiles teóricos N(0,1)", 
         ylab = "Cuantiles simulados", 
         main = paste("QQ-plot: Simulado vs N(0,1) (df =", n, ")"))
  abline(0, 1, col = "red", lwd = 2)
  
  # Estadísticas numéricas: media y varianza de Z-scores
  mean_z <- mean(z_scores)
  var_z <- var(z_scores)
  
  cat(sprintf("Grados de libertad n = %d:\n", n))
  cat(sprintf("  Media de Z-scores: %.4f (esperado: 0)\n", mean_z))
  cat(sprintf("  Varianza de Z-scores: %.4f (esperado: 1)\n", var_z))
  cat("\n")
}
...
```{r}
# Elegir grados de libertad para las dos variables chi-cuadrada independientes
k1 <- 5
k2 <- 10
k_total <- k1 + k2  # Grados de libertad de la suma

# Número de simulaciones
num_sim <- 10000

# Simular sumas de dos chi-cuadrada independientes
sums <- rchisq(num_sim, df = k1) + rchisq(num_sim, df = k2)

# Comparar con distribución chi-cuadrada teórica de k_total grados de libertad
# Histograma de las sumas simuladas
hist(sums, freq = FALSE, breaks = 50, col = "lightblue", 
     xlab = "Valor", main = paste("Suma de chi-cuadrada (", k1, "+", k2, "=", k_total, "df)"))
# Superponer densidad teórica chi-cuadrada
curve(dchisq(x, df = k_total), add = TRUE, col = "red", lwd = 2)
legend("topright", legend = c("Histograma simulado", "Densidad chi-cuadrada"), 
       fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

# QQ-plot para comparación
qqplot(qchisq(ppoints(num_sim), df = k_total), sums, 
       xlab = "Cuantiles teóricos chi-cuadrada", 
       ylab = "Cuantiles simulados", 
       main = paste("QQ-plot: Simulado vs Teórico (df =", k_total, ")"))
abline(0, 1, col = "red", lwd = 2)
...
cat(sprintf("Grados de libertad: %d + %d = %d\n", k1, k2, k_total))
cat(sprintf("Media simulada: %.4f (teórica: %.4f)\n", mean_sim, mean_theor))
cat(sprintf("Varianza simulada: %.4f (teórica: %.4f)\n", var_sim, var_theor))
```
:::
