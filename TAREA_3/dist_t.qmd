...
2.  Para la muestra de mayor tamaño, realiza un histograma, superpone la función de densidad teórica de la variable aleatoria $t(5)$ y la función de densidad de una normal estándar.
    ...
    # Valores teóricos
    theoretical_mean <- 0  # Para nu > 1
    theoretical_var <- nu / (nu - 2)  # Para nu > 2

    # Tamaños de muestra
    sizes <- c(100, 300, 1000)

    # Para cada tamaño de muestra
    for (size in sizes) {
      # Generar muestra aleatoria
      sample <- rt(size, df = nu)
      ...
      cat(sprintf("  Media muestral: %.4f (teórica: %.4f)\n", sample_mean, theoretical_mean))
      cat(sprintf("  Varianza muestral: %.4f (teórica: %.4f)\n", sample_var, theoretical_var))
      cat("\n")
    }

    # Para la muestra de mayor tamaño (1000), generar histograma con densidades superpuestas
    sample_large <- rt(1000, df = nu)
    hist(sample_large, freq = FALSE, breaks = 50, col = "lightblue", 
         xlab = "Valor", main = "Histograma de t(5) con densidades superpuestas")
    # Superponer densidad teórica de t(5)
    curve(dt(x, df = nu), add = TRUE, col = "red", lwd = 2)
    # Superponer densidad de normal estándar
    curve(dnorm(x), add = TRUE, col = "green", lwd = 2, lty = 2)
    legend("topright", legend = c("Histograma", "Densidad t(5)", "Densidad N(0,1)"), 
           fill = c("lightblue", NA, NA), col = c(NA, "red", "green"), 
...
# Generar Y ~ χ²(nu)
Y <- rchisq(n_sample, df = nu)

# Calcular T = X / sqrt(Y / nu)
T <- X / sqrt(Y / nu)

# Histograma de T
hist(T, freq = FALSE, breaks = 50, col = "lightblue", 
     xlab = "Valor", main = "Histograma de T vs Densidad teórica t(10)")

# Superponer densidad teórica de t(nu)
curve(dt(x, df = nu), add = TRUE, col = "red", lwd = 2)

legend("topright", legend = c("Histograma de T", "Densidad t(10)"), 
       fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

# Opcional: QQ-plot para comparación
qqplot(qt(ppoints(n_sample), df = nu), T, 
       xlab = "Cuantiles teóricos t(10)", 
       ylab = "Cuantiles de T", 
       main = "QQ-plot: T vs t(10)")
...
# Vector para almacenar los valores de T
T_values <- numeric(num_samples)

# Generar 1000 muestras
for (i in 1:num_samples) {
  # Generar muestra de tamaño n de N(mu,1)
  sample <- rnorm(n, mean = mu, sd = 1)
  
  # Calcular media muestral
  x_bar <- mean(sample)
  
  # Calcular desviación estándar muestral (S)
  s <- sd(sample)  # sd() en R calcula la desviación estándar muestral (con n-1)
  
  # Calcular T = (x_bar - mu) / (s / sqrt(n))
  T_values[i] <- (x_bar - mu) / (s / sqrt(n))
}

# Histograma de T
hist(T_values, freq = FALSE, breaks = 50, col = "lightblue", 
     xlab = "Valor", main = "Histograma de T vs Densidad teórica t(9)")

# Superponer densidad teórica de t(9)
curve(dt(x, df = 9), add = TRUE, col = "red", lwd = 2)

legend("topright", legend = c("Histograma de T", "Densidad t(9)"), 
       fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

# Opcional: QQ-plot para comparación
qqplot(qt(ppoints(num_samples), df = 9), T_values, 
       xlab = "Cuantiles teóricos t(9)", 
       ylab = "Cuantiles de T", 
       main = "QQ-plot: T vs t(9)")
abline(0, 1, col = "red", lwd = 2)
```
::::
