...
a)  Si $X\sim t(n)$ entonces $X^2 \sim F(1,n)$.
    ...
    ```{r}
    # Parámetros
    n <- 10  # Grados de libertad para t
    num_sim <- 10000
    ...
    # Histograma de X^2
    hist(X_squared, freq = FALSE, breaks = 50, col = "lightblue", 
         xlab = "Valor", main = paste("Histograma de X^2 vs Densidad F(1,", n, ")"))

    # Superponer densidad teórica de F(1, n)
    curve(df(x, df1 = 1, df2 = n), add = TRUE, col = "red", lwd = 2)

    legend("topright", legend = c("Histograma de X^2", paste("Densidad F(1,", n, ")")), 
           fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

    # QQ-plot para comparación
    qqplot(qf(ppoints(num_sim), df1 = 1, df2 = n), X_squared, 
           xlab = paste("Cuantiles teóricos F(1,", n, ")"), 
           ylab = "Cuantiles de X^2", 
           main = paste("QQ-plot: X^2 vs F(1,", n, ")"))
...
b)  Si $X \sim F(a,b)$ entonces $\frac{1}{X} \sim F(b,a)$.
    ...
    # Generar X ~ F(a, b)
    X <- rf(num_sim, df1 = a, df2 = b)

    # Calcular 1/X
    inv_X <- 1 / X

    # Histograma de 1/X
    hist(inv_X, freq = FALSE, breaks = 50, col = "lightblue", 
         xlab = "Valor", main = paste("Histograma de 1/X vs Densidad F(", b, ",", a, ")"))

    # Superponer densidad teórica de F(b, a)
    curve(df(x, df1 = b, df2 = a), add = TRUE, col = "red", lwd = 2)

    legend("topright", legend = c("Histograma de 1/X", paste("Densidad F(", b, ",", a, ")")), 
           fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

    # QQ-plot para comparación
    qqplot(qf(ppoints(num_sim), df1 = b, df2 = a), inv_X, 
           xlab = paste("Cuantiles teóricos F(", b, ",", a, ")"), 
           ylab = "Cuantiles de 1/X", 
           main = paste("QQ-plot: 1/X vs F(", b, ",", a, ")"))
    abline(0, 1, col = "red", lwd = 2)
    ```
...
c)  Si $X \chi^2(a)$ y $Y \sim \chi^2(b)$ son independientes, entonces $\frac{X/a}{Y/b} \sim F(a,b)$.
    ...
    # Generar X ~ χ²(a) y Y ~ χ²(b) independientes
    X <- rchisq(num_sim, df = a)
    Y <- rchisq(num_sim, df = b)

    # Calcular (X/a) / (Y/b)
    F_stat <- (X / a) / (Y / b)

    # Histograma de F_stat
    hist(F_stat, freq = FALSE, breaks = 50, col = "lightblue", 
         xlab = "Valor", main = paste("Histograma de (X/a)/(Y/b) vs Densidad F(", a, ",", b, ")"))

    # Superponer densidad teórica de F(a, b)
    curve(df(x, df1 = a, df2 = b), add = TRUE, col = "red", lwd = 2)

    legend("topright", legend = c("Histograma de (X/a)/(Y/b)", paste("Densidad F(", a, ",", b, ")")), 
           fill = c("lightblue", NA), col = c(NA, "red"), lty = c(NA, 1), border = c("black", NA))

    # QQ-plot para comparación
    qqplot(qf(ppoints(num_sim), df1 = a, df2 = b), F_stat, 
           xlab = paste("Cuantiles teóricos F(", a, ",", b, ")"), 
           ylab = "Cuantiles de (X/a)/(Y/b)", 
           main = paste("QQ-plot: (X/a)/(Y/b) vs F(", a, ",", b, ")"))
    abline(0, 1, col = "red", lwd = 2)
    ```
:::
