...
3.  Para el tamaño de muestra más grande, genera un histograma de la muestra junto con la curva de la PDF teórica superpuesta.
    ...
    # Definir tamaños de muestra desde 100 hasta 10000 en pasos de 100
    sizes <- seq(100, 10000, by = 100)

    # Inicializar vectores para almacenar medias y varianzas muestrales
    medias_muestrales <- numeric(length(sizes))
    varianzas_muestrales <- numeric(length(sizes))

    # Generar muestras y calcular estadísticas para cada tamaño
    set.seed(123)  # Para reproducibilidad
    for (i in 1:length(sizes)) {
      muestra <- rbeta(sizes[i], shape1 = alpha, shape2 = beta)
      medias_muestrales[i] <- mean(muestra)
      varianzas_muestrales[i] <- var(muestra)
    }

    # Gráfica de convergencia de la media muestral a la media teórica
    plot(sizes, medias_muestrales, type = "l", col = "blue", 
    ...
    # Gráfica de convergencia de la varianza muestral a la varianza teórica
    plot(sizes, varianzas_muestrales, type = "l", col = "green", 
         xlab = "Tamaño de muestra", ylab = "Varianza muestral", 
         main = "Convergencia de la varianza muestral")
    abline(h = varianza_teorica, col = "red", lty = 2)
    legend("topright", legend = c("Varianza muestral", "Varianza teórica"), 
           col = c("green", "red"), lty = c(1, 2))

    # Para el tamaño de muestra más grande (10000), generar histograma con PDF teórica superpuesta
    muestra_grande <- rbeta(10000, shape1 = alpha, shape2 = beta)
    hist(muestra_grande, freq = FALSE, col = "lightblue", 
         xlab = "Valor", ylab = "Densidad", 
         main = "Histograma de la muestra (n=10000) con PDF teórica")
...
# Definir tamaños de muestra para verificar TCL
sizes <- c(10, 30, 100, 500)

# Número de réplicas para cada tamaño de muestra
num_replicas <- 1000

# Configurar para múltiples gráficos con márgenes ajustados para dar más espacio
par(mfrow = c(2, 2), mar = c(5, 4, 4, 8) + 0.1)  # Aumentar margen derecho para la leyenda

# Para cada tamaño de muestra, generar réplicas y graficar
set.seed(123)  # Para reproducibilidad
for (n in sizes) {
  # Generar num_replicas medias muestrales
  medias_muestrales <- replicate(num_replicas, mean(rbeta(n, shape1 = alpha, shape2 = beta)))
  
  # Calcular la desviación estándar teórica de la media muestral (sqrt(var/n))
  sd_teorica <- sqrt(varianza_teorica / n)
  
  # Crear histograma de las medias muestrales
  hist(medias_muestrales, freq = FALSE, col = "lightblue", 
       xlab = "Media muestral", ylab = "Densidad", 
       main = paste("TCL para n =", n), xlim = c(0, 0.6))
  
  # Superponer la curva de la distribución normal teórica
  curve(dnorm(x, mean = media_teorica, sd = sd_teorica), 
        add = TRUE, col = "red", lwd = 2)
  
  # Agregar leyenda con tamaño de texto reducido y sin caja (o con caja más pequeña)
  legend("topright", legend = c("Histograma de medias", "Normal teórica"), 
         col = c("lightblue", "red"), lty = c(1, 1), lwd = c(1, 2), 
         cex = 0.7, bty = "n")  # cex=0.7 hace el texto más pequeño, bty="n" quita la caja
}
