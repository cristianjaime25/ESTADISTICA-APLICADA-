...
Primero, leemos el archivo shapefile para Coahuila:

```{r entidades}
# Nota: Asegúrate de que todos los archivos (shp, shx, dbf, prj, cpg) estén en el mismo directorio: data

coahuila_estado <- st_read("data/05ent.shp", options = "ENCODING=LATIN1", quiet = TRUE)
```
...
mapa_coahuila <- ggplot(coahuila_estado) +
...
```{r municipios}
municipios_coahuila <- st_read("data/05mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

# Explorar los datos
cat("Número de municipios en Coahuila:", nrow(municipios_coahuila), "\n")
cat("Primeros 10 municipios:\n")
print(municipios_coahuila$NOMGEO[1:10])
```
...
municipios_importantes <- c("Saltillo", "Torreón", "Monclova", 
                          "Acuña", "Matamoros", "San Pedro")
...
mapa_destacados <- ggplot(municipios_coahuila) +
  ...
  labs(
    title = "Municipios Importantes de Coahuila",
...
municipios_grandes <- municipios_coahuila |> 
  st_drop_geometry() |> 
  arrange(desc(area_km2)) |> 
  select(NOMGEO, area_km2) |> 
  head(10)

print("Los 10 municipios más grandes de Coahuila:")
...
mapa_areas <- ggplot(municipios_coahuila) +
  ...
  labs(
    title = "Área de los Municipios de Coahuila",
