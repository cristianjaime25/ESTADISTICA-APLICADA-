...
```{r}
# Chunk 1: Probabilidad de Obtener un Número Par

# 1. Definición del Espacio Muestral y el Evento
espacio_muestral <- 1:8
evento_par <- espacio_muestral[espacio_muestral %% 2 == 0] # Números pares: 2, 4, 6, 8

# 2. Cálculo de la Probabilidad Teórica
total_resultados <- length(espacio_muestral)
casos_favorables <- length(evento_par)

probabilidad_par_teorica <- casos_favorables / total_resultados

cat("--- 1. Probabilidad de Obtener un Número Par ---\n")
cat(sprintf("Espacio Muestral: %s\n", paste(espacio_muestral, collapse = ", ")))
cat(sprintf("Casos Favorables (pares): %s\n", paste(evento_par, collapse = ", ")))
cat(sprintf("Probabilidad Teórica (4/8): %.2f\n", probabilidad_par_teorica))

# Verificación (simulación de una única extracción)
# resultado_simulado <- sample(espacio_muestral, size = 1)
# cat(sprintf("Resultado de una extracción simulada: %d\n", resultado_simulado))
cat("----------------------------------------------\n")
```
:::
...
```{r}
# Chunk 2: Media y Varianza de Uniforme Discreta [5, 15]

# 1. Definición de Parámetros
a <- 5
b <- 15
N <- b - a + 1 # Número total de posibles resultados (11)

# 2. Cálculo de la Media Teórica
media_teorica <- (a + b) / 2

# 3. Cálculo de la Varianza Teórica
varianza_teorica <- (N^2 - 1) / 12

cat("--- 2. Media y Varianza de Uniforme Discreta [5, 15] ---\n")
cat(sprintf("Valores posibles: %d a %d (N = %d)\n", a, b, N))
cat(sprintf("Media Teórica ( (a+b)/2 ): %.2f\n", media_teorica))
cat(sprintf("Varianza Teórica ( (N^2-1)/12 ): %.2f\n", varianza_teorica))
cat("--------------------------------------------------------\n")
...
```{r}
# Chunk 3: Simulación de Uniforme Discreta en {0, 1, 2, 3, 4}

# 1. Definición de Parámetros
valores_posibles <- 0:4
N_simulaciones <- 500
N_categorias <- length(valores_posibles) # 5

# 2. Simulación de 500 Observaciones
set.seed(42) # Para reproducibilidad
observaciones <- sample(valores_posibles, size = N_simulaciones, replace = TRUE)

# 3. Frecuencias Observadas (Contar las ocurrencias de cada valor)
frecuencias_observadas <- table(observaciones)
frecuencias_df <- as.data.frame(frecuencias_observadas)
colnames(frecuencias_df) <- c("Valor", "Frecuencia_Observada")

# 4. Frecuencias Teóricas
prob_teorica_unit <- 1 / N_categorias # 0.20
frecuencia_teorica <- N_simulaciones * prob_teorica_unit # 100

# 5. Comparación
frecuencias_df$Frecuencia_Teorica <- frecuencia_teorica
frecuencias_df$Prob_Teorica <- prob_teorica_unit
frecuencias_df$Prob_Observada <- frecuencias_df$Frecuencia_Observada / N_simulaciones

cat("--- 3. Simulación Uniforme Discreta {0, 1, 2, 3, 4} (N=500) ---\n")
cat(sprintf("Probabilidad Teórica por Valor: %.2f\n", prob_teorica_unit))
cat(sprintf("Frecuencia Esperada por Valor: %.0f\n\n", frecuencia_teorica))

print(frecuencias_df)

# Opcional: Gráfico de Barras para Visualización
# ====================================================================
barplot(frecuencias_df$Frecuencia_Observada,
        names.arg = frecuencias_df$Valor,
        main = "Frecuencias Observadas vs. Teóricas (N=500)",
        xlab = "Valor (k)",
        ylab = "Frecuencia",
        ylim = c(0, max(frecuencias_df$Frecuencia_Observada) * 1.1),
        col = "#0072B2")

# Agregar la línea de Frecuencia Teórica (y = 100)
abline(h = frecuencia_teorica, col = "#D55E00", lwd = 2, lty = 2)

legend("topright",
       legend = c("Frecuencias Observadas", "Frecuencia Teórica (100)"),
       fill = c("#0072B2", NA),
       lty = c(NA, 2),
       lwd = c(NA, 2),
       col = c(NA, "#D55E00"),
