...
```{r}
#| code-fold: true
geometrica_binneg <- function(r = 5, p = 0.25, n_sim) { 
  ...
  ggplot(data_hist, aes(x = valores, fill = distribucion)) +
    ...
    labs(title = "Comparación de Simulaciones", 
         subtitle = paste("Probabilidad =", p, "| Simulaciones =",
...
geometrica_binneg(r = 5, p = 0.25, n_sim = 1500)
```
:::
...
p <- 0.65
n_sim <- 1500
# a) Modelado mediante suma de distribuciones geométricas
set.seed(123)
sumgeom_f <- replicate(n_sim, {
  sum(rgeom(r, prob = p))
})
sim_prob_f <- mean(sumgeom_f)
# b) Cálculo del valor esperado de intentos fallidos
prob_f <- r * (1-p) / p
desv_std_f <- sqrt(r * (1-p)/(p^2))
cat("Valor esperado de intentos fallidos (teórico):", round(prob_f, 2),"±", round(desv_std_f, 2), "\n")
cat("Valor esperado de intentos fallidos (simulación):", round(sim_prob_f,2),"\n\n")
cat("Diferencia absoluta:", abs(prob_f - sim_prob_f),"\n\n")
# Visualización de la Distribución
ggplot(data.frame(Intentos = sumgeom_f), aes(x = Intentos)) +
  geom_histogram(aes(y = after_stat(density)), bins = 25, fill = "#8B4789", alpha = 0.65) +
  geom_vline(xintercept = prob_f, color = "#FF6B35", 
             linetype = "dashed", linewidth = 1.2) +
  labs(title = "Distribución de intentos fallidos durante el entrenamiento",
       subtitle = paste("Probabilidad =", p, "| Éxitos requeridos =", r, 
                       "| Línea naranja: promedio =", round(prob_f, 1)),
       x = "Cantidad de intentos fallidos",
...
p <- 0.35
k <- 10
# a) Probabilidad de obtener exactamente 10 pruebas no exitosas
prob_fexacto_10 <- dnbinom(k, size = r, prob = p)
# b) Probabilidad de requerir más de 18 pruebas no exitosas
prob_fmas_18 <- 1 - pnbinom(18, size = r, prob = p)
# c) Con un costo unitario de $12,500 por prueba, determinar el gasto esperado
costo_prueba <- 12500
...
ggplot(data.frame(fallos = sum_fallidas), aes(x = fallos)) +
  geom_histogram(aes(y = after_stat(density)), bins = 35, fill = "#2D6A4F", alpha = 0.65) +
  geom_vline(xintercept = costo_fallidas, color = "#E63946", 
             linetype = "dashed", linewidth = 1.3) +
  labs(title = "Distribución de ensayos no exitosos en experimentos de laboratorio",
       subtitle = paste("Probabilidad =", p, "| Éxitos necesarios =", r, 
                       "| Línea roja: valor medio =", round(costo_fallidas, 1)),
       x = "Cantidad de ensayos fallidos",
       y = "Densidad de frecuencia") +
  theme_minimal()
cat("Probabilidad de exactamente 10 ensayos fallidos (analítico):", round(prob_fexacto_10, 6), "\n")
cat("Probabilidad de superar 18 ensayos fallidos (analítico):", round(prob_fmas_18, 6), "\n")
cat("Promedio de ensayos fallidos (Monte Carlo):", round(prob_sim, 6), "\n")
cat("Gasto esperado en ensayos no exitosos: $", format(round(costo_esperado, 2), big.mark = ","), "\n")
cat("Desviación (Teórico vs Simulación):", abs(costo_fallidas - prob_sim),"\n\n")
```
